...
use Illuminate\Support\Arr;
    ...
    if (isset($_POST["metricaGraf"])) {
        ...
        $strSQLView = "SELECT nombre FROM {$BaseDatos_general}.vistas_generadas where id_estrat = {$Estrat} and nombre like '%{$mt}%';";
    ...
    if(isset($_POST['booCamordCamposBd'])){
        ...
        $Lsql = "SELECT PREGUN_ConsInte__b, PREGUN_Tipo______b, PREGUN_Texto_____b FROM ".$BaseDatos_systema.".PREGUN WHERE PREGUN_ConsInte__GUION__b = ".$intIdBd." AND PREGUN_Tipo______b IN (1,3,4,5) AND PREGUN_Texto_____b NOT IN ('ORIGEN_DY_WF','OPTIN_DY_WF','ESTADO_DY')";
        $res_Resultado = $mysqli->query($Lsql);
        ...
        if($src_Tipificaciones->num_rows > 0){
            while($key = $src_Tipificaciones->fetch_object()){
                if(isset($_POST['noMuestreEsta'])  && $_POST['noMuestreEsta'] != '0' && $_POST['noMuestreEsta'] != ' '){
                    if($key->LISOPC_ConsInte__b != $_POST['noMuestreEsta']){
                       $options .= "<option value='".$key->LISOPC_ConsInte__b."'>".$key->LISOPC_Nombre____b."</option>"; 
                    }
                }else{
                    $options .= "<option estado='{$key->reintento}' value='{$key->LISOPC_ConsInte__b}'>{$key->LISOPC_Nombre____b} / {$key->reintento}</option>";
                }
    ...
    if(isset($_GET['getIVRList'])){
        $proyecto = (isset($_POST["proyecto"])) ? $_POST["proyecto"] : null;
        $datos = [];

        if(isset($proyecto) && $proyecto != null){
                
            $strSQLIvrs = "SELECT I.id, I.nombre_usuario_ivr FROM {$BaseDatos_telefonia}.dy_ivrs I JOIN {$BaseDatos_telefonia}.dy_proyectos P ON P.id = I.id_proyecto  WHERE P.id_huesped = '{$proyecto}'";
            $resSQLIvrs = $mysqli->query($strSQLIvrs);
            


            if($resSQLIvrs){
                if($resSQLIvrs->num_rows > 0){
                    while ($objSQLIvrs = $resSQLIvrs->fetch_object()) {
                        array_push($datos, ["nameIvr" => $objSQLIvrs->nombre_usuario_ivr, "idIvr" => $objSQLIvrs->id]);
                    }
                }

            }

            $message = ["state" => "ok", "message" => $datos];
        }else{
            $message = ["state" => "error"];
        }
