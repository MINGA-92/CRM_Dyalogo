...
<script>
    ...
    $("#BtnGenerarReporte").click(function(){
        ...
        $.ajax({
            ...
            success: function(php_response) {
                Respuesta= php_response.msg;
                ResultadoI= php_response.Respuesta;
                ResultadoD= ResultadoI.replace("   ", " ");
                Resultado= ResultadoD.replace("  ", " ");
                console.log("Resultado: ", Resultado);
                ...
                ArrayCondiciones= Condiciones.split(" ");
                DimensionArray= ArrayCondiciones.length;
                /*console.log("ArrayConsulta: ", ArrayConsulta);
                console.log("ArrayUpdate: ", ArrayUpdate);
                console.log("BaseDatos: ", BaseDatos);
                console.log("Condiciones: ", Condiciones);
                console.log("ArrayCondiciones: ", ArrayCondiciones);
                console.log("DimensionArray: ", DimensionArray);*/

                var NuevoArrayCondiciones= [];
                for (let d = 0; d < DimensionArray; d++) {
                    const Objeto = ArrayCondiciones[d];
                    //console.log("Objeto: ", Objeto);

                    //Identificar Si Es Columna o Dato
                    NumeroLetrasObj= Objeto.length;
                    //console.log("NumeroLetrasObj: ", NumeroLetrasObj);
                    if(NumeroLetrasObj > 17) {
                        ColumnaBD= ArrayCondiciones[d];
                        //Identificar Si Falta Nombre
                        NumeroLetras= ColumnaBD.length;
                        //console.log("NumeroLetras: ", NumeroLetras);
                        if(NumeroLetras < 19) {
                            PosicionLineas= ColumnaBD.indexOf('_');
                            PosicionCorte= PosicionLineas+1;
                            CorteColumna= ColumnaBD.substring(0, PosicionCorte);
                            CorteColumna_2= ColumnaBD.substring(PosicionCorte, NumeroLetras);
                            NumeroLetras2= BaseDatos.length;
                            PosicionLinea2= BaseDatos.lastIndexOf('_');
                            CorteBD= BaseDatos.substring(PosicionLinea2+1, NumeroLetras2);
                            Columna= CorteColumna+CorteBD+CorteColumna_2;
                            //console.log("Columna: ", Columna);
                            NuevoArrayCondiciones.push(Columna);

                        }else if(NumeroLetras == 21) {
                            PosicionLineas= ColumnaBD.indexOf('_');
                            PosicionCorte= PosicionLineas+1;
                            CorteColumna= ColumnaBD.substring(0, PosicionCorte);
                            CorteColumna_2= ColumnaBD.substring(PosicionCorte+1, NumeroLetras);
                            Columna= CorteColumna+CorteColumna_2;
                            //console.log("Columna: ", Columna);
                            NuevoArrayCondiciones.push(Columna);
                        }else{
                            NuevoArrayCondiciones.push(ColumnaBD);
                        }
                    }else{
                        NuevoArrayCondiciones.push(Objeto);
                    }
                    //console.log("NuevoArrayCondiciones: ", NuevoArrayCondiciones);
                }
                

                Condiciones= "";
                for (let i = 0; i < NuevoArrayCondiciones.length; i++) {
                    const element = NuevoArrayCondiciones[i];
                    Condiciones= Condiciones+NuevoArrayCondiciones[i]+" ";
                }
                console.log("Condiciones Final: ", Condiciones);


                //Identificar Si Son 2 Bases De Datos
                if(Inner == "INNER"){
                ...
                if(ContadorL > 1){
                    for (let i= 0; i < Condiciones.length; i++) {
                        ...
                        if((Letra_1 == "-") && (Letra_2 == "-" )){
                            ...
                            if(LetraCambiar == "=" ){
                                //console.log("LetraCambiar: ", LetraCambiar);
                                ...
                                //Primero Estado Luego Fecha
                                if(Pos1 == 68) {
                                    //console.log(" ------- > Primero Estado Luego Fecha <--------- ");
                                    Dia= Condiciones.substr(Pos1-1, Pos2-1);
                                    NewDia= parseInt(Dia) + 1;
                                    console.log("NewDia: ", NewDia);
                                    Rango_2= Rango_2.trim();
                                    DimensionRango= Rango_2.length;
                                    PosicionCorte= DimensionRango - 3;
                                    if (Dia => 30) {
                                        NewDia= parseInt("01");
                                        Mes= Rango_2.substr(PosicionCorte-3, PosicionCorte);
                                        NewMes= parseInt(Mes) + 1;
                                        if (NewMes > 30) {
                                            NewMes= parseInt("01");
                                        }
                                        var FechaNew= Rango_2.substr(0, PosicionCorte-4);
                                        var FechaGenerica= FechaNew+"-"+NewMes+"-"+NewDia+"'";
                                    }else{
                                        var FechaNew= Rango_2.substr(0, PosicionCorte);
                                        var FechaGenerica= FechaNew+NewDia+"'";
                                    }
                                    //console.log("FechaNew: ", FechaNew);
                                    //console.log("FechaGenerica: ", FechaGenerica);
                                ...
                                //Primero Fecha Luego Estado
                                }else{
                                    //console.log(" ------- > Primero Fecha Luego Estado <--------- ");
                                    Rango_2= Rango_2.trim();
                                    DimensionRango= Rango_2.length;
                                    //console.log("DimensionRango: ", DimensionRango);
                                    for (let i= 0; i < DimensionRango; i++) {
                                        var Posicion= i;
                                        const Letra_1= Rango_2[Posicion];
                                        const Letra_2= Rango_2[Posicion+3];
                                        if((Letra_1 == "-") && (Letra_2 == "-" )){
                                            PosicionCorte= Posicion + 4;
                                            Dia= Rango_2.substr(PosicionCorte, PosicionCorte+2);
                                            NewDia= parseInt(Dia) + 1;
                                            if (Dia => 30) {
                                                NewDia= parseInt("01");
                                                Mes= Rango_2.substr(PosicionCorte-3, PosicionCorte);
                                            ...
                                            var Rango_3= Rango_2.substr(0, PosicionCorte+3);
                                            var Rango_4= Rango_2.substr(PosicionCorte+3, DimensionRango);
                                            var Condiciones= Rango_1+" BETWEEN "+Rango_3+" AND "+FechaGenerica+" "+Rango_4;
                                            
                                            if(PosicionCorte > DimensionRango){
                                                console.log("PosicionCorte: ", PosicionCorte);
                                                console.log("DimensionRango: ", DimensionRango);
                                                break;
                                            }
