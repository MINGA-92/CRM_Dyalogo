<?php 
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
        ...
        if(isset($_GET['insertarDatos']) && isset($_POST["oper"])){
            ...
            if($_POST["oper"] == "add"){
                ...
                $tipoIntegracion = 'facebook';

                $titulo = $nombre;
                $mensajeBienvenida = '';
                $campoBusqueda = (isset($_POST['campoBusquedaFb'])) ? $_POST['campoBusquedaFb'] : 0;
                ...
                //Guardamos tambien en facebook(El consumo para hacer la actualizacion en middleware)
                $ws = "UPDATE dy_facebook.canales SET id_cfg_chat = {$id_chat} WHERE identificador_aplicacion = '{$datoIntegracion}'";
                $mysqli->query($ws);
            ...
            $syncCanales = json_decode(syncFacebook($datoIntegracion, $id_chat));
    ...
    function agregarConfiguracionBot($id_chat, $accionDetalle){
        ...
        if($a_res && $a_res->num_rows > 0){
            ...
            if(isset($_POST['campoBusquedaFb']) && $_POST['campoBusquedaFb'] != '' && $_POST['campoBusquedaFb'] != 0){

                // Actualizamos la tabla bot con el ani
                $upd = "UPDATE {$dyalogo_canales_electronicos}.dy_bot SET id_pregun_dato_facebook = " . $_POST['campoBusquedaFb'] . " WHERE id = " . $autoRes->id_bot;
                $mysqli->query($upd);
            }
    ...
    function formatearCanales($canales, $chatActual){
        global $mysqli;
        ...
        foreach ($canales as $key => $value) {
            if($value->id_cfg_chat == 0){
                $formated[$key]['nombre'] = $value->nombre;
                $formated[$key]['cuenta'] = $value->identificador_aplicacion;
                $formated[$key]['disponible'] = true;
            }else if($value->id_cfg_chat == $chatActual){
                $formated[$key]['nombre'] = $value->nombre. '- Usado en este paso';
                $formated[$key]['cuenta'] = $value->identificador_aplicacion;
                $formated[$key]['disponible'] = true;
            }else{
                ...
                $formated[$key]['nombre'] = $value->nombre.' (Está siendo usado en:'.$rowEsto->estrategia.', Paso:'.$rowEsto->paso.')';
                $formated[$key]['cuenta'] = $value->identificador_aplicacion;
    ...
    function obtenerCanales(){
        ...
        return conexionApi('https://10.142.0.13:3001/dymdw/api/config/f/getChannels', $data);
    }
    ...
    function syncFacebook($canal, $chatId){
        global $URL_SERVER;
        ...
        return conexionApi('https://10.142.0.13:3001/dymdw/api/config/f/sync_facebook', $data);
    }
