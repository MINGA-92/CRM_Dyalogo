...
// Esta funcion se copio de G2_CRUD, si se realiza cambio desde aqui modificar la del origen(G2_CRUD) tambien
// TODO: Centralizar esta funcion en un solo lado en donde retorne la data en un solo tipo de dato
...
function countAgentesCampana($idCampan){
    global $mysqli;
    $sql=$mysqli->query("SELECT id_agente FROM dyalogo_telefonia.dy_campanas_agentes WHERE id_campana = (SELECT id FROM dyalogo_telefonia.dy_campanas WHERE id_campana_crm={$idCampan})");
    if($sql){
        return $sql->num_rows;
    }
    return 0;
...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if (isset($_POST["oper"]) && isset($_GET['insertarDatosGrilla'])) {
        ...
        if ($_POST["oper"] == 'add') {
            if(isset($_GET["infoWS"])){ 
                $_SESSION['HUESPED']=$_POST['strHuesped_t'];
                $_SESSION['IDENTIFICACION']=$_POST['strIdentificacion_t'];
                $_SESSION['CORREO']=$_POST['strCorreo_t'];
                $_SESSION['PROYECTO']=$_POST['strProyecto_t'];
            }
            ...
            if (isset($_POST['GenerarFromExel'])) {
                ...
                $nombreScript = removeSpacesCharacters($_POST['G10_C71']) . " - BD";
                ...
                $nombreScript = removeSpacesCharacters($_POST['G10_C71']) . " - SCRIPT";
            ...
            if ($res_ValidaCampan && $res_ValidaCampan->num_rows < 1) {
                ...
                if ($mysqli->query($Lsql) === true) {
                    ...
                    $str_LsqlI = "INSERT INTO " . $BaseDatos_systema . ".G10(G10_C73, G10_C74, G10_C72 , G10_C75, G10_C78, G10_C80, G10_C92 , G10_C93, G10_C94, G10_C99, G10_C109, G10_C110, G10_C112, G10_C113, G10_C115, G10_C116, G10_C118, G10_C119, G10_C121, G10_C122, G10_C76, G10_C77, G10_C108, G10_C111, G10_C114, G10_C117, G10_C120, G10_C71, G10_C126, G10_C127, G10_C128, G10_C129, G10_C130, G10_C131 , G10_C123, G10_C124, G10_C125, G10_C79, G10_C102, G10_C326)";
                    $str_LsqlV = " VALUES ('" . $id_scrip . "', '" . $id_Guion . "', -1, " . $id_Muestras . ", -1, 5, 0, 10, 30, 3, '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', 5, -1 ,  -1, -1, -1, -1, -1, '" . removeSpacesCharacters($_POST['G10_C71']) . "', -1, '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', -1, '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', -1, '" . date('Y-m-d') . " 00:00:01', '" . date('Y-m-d') . " 23:59:59', '-1', '90', '25')";
                    ...
                    if ($mysqli->query($Lsql_Insertar) === TRUE) {
                        ...
                        if(isset($_GET["infoWS"])){ 
                            echo json_encode(array("id_campan" => $id_usuario, "id_campan_cbx" => $dyCampanId));
                            exit();
                        }
        ...
        if (isset($_POST["G10_C71"])) {
            ...
            $str_LsqlU .= $separador . "G10_C71 = '" . removeSpacesCharacters($_POST["G10_C71"]) . "'";
            $str_LsqlI .= $separador . "G10_C71";
            $str_LsqlV .= $separador . "'" . removeSpacesCharacters($_POST["G10_C71"]) . "'";
            $validar = 1;
        }
        ...
        //este es tipo check primeo si viene y de acuerdo a su valor se pone 1 o 0
        $G10_C85=isset($_POST["G10_C85"]) ? -1 : 0;
        $separador = "";
        if ($validar == 1) {
            $separador = ",";
        }
        ...
        if (isset($_POST["G10_C93"])) {
            if ($_POST["G10_C93"] != '') {
                ...
                if(isset($_POST['G10_C76']) && intval($_POST['G10_C76']) > 5){
                    $G10_C93=countAgentesCampana($_POST["id"])+intval($_POST['G10_C92']);
                }
        ...
        if (is_numeric($tipo_p)) {
            if ($tipo_p<3 || $tipo_p == 5 || $tipo_p == 10 || $tipo_p == 14) {
                if ($operador_p == "=") {
                    ...
                    $CON .= "< '".$valor_p."' ";
                }else if ($operador_p == 'MayorMes'){
                    $CON .= "> DATE_SUB(NOW(), INTERVAL '".$valor_p."' MONTH) "; 
                }else if ($operador_p == 'MenorMes'){
                    $CON .= "< DATE_SUB(NOW(), INTERVAL '".$valor_p."' MONTH) "; 
                }else if ($operador_p == 'MayorDia'){
                    $CON .= "> DATE_SUB(NOW(), INTERVAL '".$valor_p."' DAY) "; 
                }else if ($operador_p == 'MenorDia'){
                    $CON .= "< DATE_SUB(NOW(), INTERVAL '".$valor_p."' DAY) "; 
                }
    ...
    if (isset($_GET['administrar_base'])) {
        ...
        if ($resSQLCampana_t->num_rows>0) {
            ...
            if ($intEstado_t > 0 || $intTipificacion_t > 0) {
                ...
                if ($intTipificacion_t > 0) { 
                        ...
                        $strTipificacion_t = "SELECT MONOEF_ConsInte__b FROM " . $BaseDatos_systema . ".MONOEF
                            ...
                            WHERE LISOPC.LISOPC_Clasifica_b = ".$intTipificacion_t;
            ...
            if ($intReintento_t > 0 || $intActivo_t > -2 || $intAgente_t > -2) {
                ...
                if ($intActivo_t > -2) {
                    if($intActivo_t == -1){
                        // SI SE ESTA ACTIVANDO TOCA ACTUALIZA LA FECHA DE REACTIVACION
                        $strUPDATE_M_t .= " G".$intBase_t."_M".$intMuestra_t."_FechaReactivacion_b = NOW(),";
                    }
