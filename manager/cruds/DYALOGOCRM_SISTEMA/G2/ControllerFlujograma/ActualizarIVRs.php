...
if($ResultadoSQL = $mysqli->query($ActualizarSQL)) {
    ...
    $ActualizarSQL2= "UPDATE dyalogo_telefonia.dy_ivrs SET nombre_interno_ivr= '". $NombreInterno ."', nombre_usuario_ivr= '". $NombreOpcion ."', id_audio_bienvenida= '". $GrabacionBienvenida ."', id_audio_toma_digitos= '". $GrabacionDigitos ."', tiempo_espera_digitos= '". $TiempoEspera ."', nombre_raiz= '". $NombreIVR ."', id_proyecto= '". $IdProyecto ."', id_estpas= '". $IdEstrategia ."', saludo_permite_marcar_digitos= '". $AceptarDigitos ."', intentos_errados_permitidos= '". $IntentosPermitidos ."', id_audio_opcion_errada='". $GrabacionOpcErrada ."' WHERE id= '". $IdIVR ."';";
