...
if ($ResultadoOpcion = $mysqli->query($ConsultaOpcion)) {
    ...
    if($CantidadResultados > 0) {
        ...
        $ActualizarSQL= "UPDATE dyalogo_telefonia.dy_opciones_ivrs SET id_opcion= '". $IdOpcion ."', nombre_interno_opcion= '". $OpcionNumero ."', nombre_usuario_opcion='". $NombreOpcion ."', valida='". $OpcionValida ."' WHERE id= '". $IdOpcion . "';";
...
//Actualizar Flecha
if($Comprobar == "Ok"){
    $ActualizarSQL= "UPDATE DYALOGOCRM_SISTEMA.CONEXIONES_IVRS SET NOMBRE='". $NombreOpcion ."', ID_OPCION= '". $IdOpcion . "' WHERE ID_DESDE= '". $IdFrom . "' AND ID_HASTA= '". $IdTo . "';";
    if ($ResultadoSQL= $mysqli->query($ActualizarSQL)) {
