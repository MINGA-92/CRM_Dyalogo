...
class Estrategia
{
    ...
    {
        $consulta = "SELECT COUNT(1) AS veces, {$this->getLlave()} AS llave, MAX(G{$this->getIdBD()}_ConsInte__b) AS id_registroAConservar FROM DYALOGOCRM_WEB.G{$this->getIdBD()} WHERE {$this->getLlave()} <> '' AND G{$this->getIdBD()}_ConsInte__b <> '-1' AND {$this->getLlave()} IS NOT NULL GROUP BY {$this->getLlave()} HAVING veces > 1";
    ...
    private function buscarRepetidoID(array $repetido)
    {
        ...
        $consulta = "SELECT G{$this->getIdBD()}_ConsInte__b AS id_registro_bd FROM DYALOGOCRM_WEB.G{$this->getIdBD()} WHERE {$this->getLlave()} = '{$repetido['llave']}' AND G{$this->getIdBD()}_ConsInte__b <> '{$repetido['id_registroAConservar']}' ";
