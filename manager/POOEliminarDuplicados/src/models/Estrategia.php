...
class Estrategia
{
    ...
    {
        $consulta = "SELECT COUNT(1) AS veces, {$this->getLlave()} AS llave, MAX(G{$this->getIdBD()}_ConsInte__b) AS id FROM DYALOGOCRM_WEB.G{$this->getIdBD()} WHERE {$this->getLlave()} <> '' AND G2318_ConsInte__b <> '-1' AND {$this->getLlave()} IS NOT NULL GROUP BY {$this->getLlave()} HAVING veces > 1";
    ...
    private function calcularValoresBD(string $bd, array $repetido)
    {
        ...
        $this->updateCamposBD($bd, $repetido, $result);
    ...
    protected function calcularValoresMuestra(string $muestra, array $repetido)
    {
        ...
        echo empty($result) === false ? $this->updateCamposMuestra($muestra, $repetido, $result) : false;
        // echo "\n empty ", empty($result), "\n";
